...
from app.services import audit_service, comment_service, project_service

router = APIRouter(tags=["comments"])


def _load_project(db: Session, actor: Actor, workspace_slug: str, slug: str, scope: str) -> Project:
    ...
    if actor.token is not None:
        ...
        return project
    # Session user: commenting requires workspace membership (any role).
    is_admin = actor.user is not None and actor.user.is_platform_admin
    if not is_admin and (
        actor.user_id is None
        or perms.get_role(db, actor.user_id, project.workspace_id) is None
    ):
...
def list_comments(
    ...
    project = _load_project(db, actor, workspace_slug, slug, "comments:read")
...
def create_comment(
    ...
    project = _load_project(db, actor, workspace_slug, slug, "comments:write")
